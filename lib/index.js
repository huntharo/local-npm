...
module.exports = (options, callback) => {
    ...
    function setCacheResponseHeaders(res, etag) {
    ...
    async function getDocumentAsync(name) {
        try {
            // If we have the data cached in memory, return it
            const cachedDoc = memoryCache.get(name);
            if (cachedDoc !== null) {
                return cachedDoc;
            }
            // If we have the package data locally, return it
            const doc = await skimLocal.get(name);
            // Cache the item for 30 seconds
            // There are multiple reads for the same doc during
            // a run of `npm i`
            memoryCache.put(name, doc, 30000);
            return doc;
        } catch (err) {
    ...
    async function VersionInfoAsync(req, res) {
        try {
            ...
            if (versionMetadata) {
                setCacheResponseHeaders(res, doc._rev);
